...
1. 別のツールで平文候補を生成して、コピーあるいはエクスポートする。
3. 本ツールに貼り付ける。
...
- [Caesar Cipher Breaker - シーザー暗号解読ツール（Day008）](https://github.com/ipusiron/caesar-cipher-breaker)で採用した形式。
...
- [Substitution Mapping Mixer - 換字式暗号の置換表の手動解析支援ツール(Day105)](https://github.com/ipusiron/substitution-mapping-mixer)で採用した形式。
...
**ブロック先頭**にある `key=value` 形式の連続した行はメタデータとして認識され、**意味評価から除外**されます（表示とコピーには含まれます）。

```
<キー名>=<値>
```

* **キー名**：英数字またはアンダースコア（例：`shift`、`branch`、`mapping`、`score`、`custom_field`）
* **値**：`=` の後のすべての文字列

#### 特徴

* 任意のキー名に対応（決め打ちではない）
* **ブロック先頭の連続した行のみ**がメタデータとして認識される
* 平文の途中にある `word=value` 形式は除外されない（誤検出防止）
* 複数のメタデータ行を含むブロックにも対応
* メタデータがゼロ個のブロックも正常に処理
* メタデータのみで平文がないブロックは無視されます
* **メタデータの値を変更してもスコアは変わりません**（同じ平文 = 同じスコア）

### 補足

* 空のブロックは無視されます
* 評価前に、ブロック内の改行はスペースに正規化されます
* コピーボタンはメタデータを含む元のブロック全体をコピーします
...
本ツールで使用しているエンベディングモデル（all-MiniLM-L6-v2）には以下の制限があります。

#### スコア差が小さい場合がある

自然な英語と意味不明な文字列のスコア差が小さい場合があります（例：0.64 vs 0.65）。このため、**順位は絶対的な正解ではなく、判断の参考**として利用してください。

#### 平文が長いほど精度が向上

エンベディングモデルは、テキストが長いほど意味的な特徴を捉えやすくなります。

| 平文の長さ | 精度の傾向 |
| --- | --- |
| 短い（数語〜1文） | スコア差が小さく、順位が不安定になりやすい |
| 中程度（2〜3文） | ある程度の精度が期待できる |
| 長い（複数文） | 自然な文章と意味不明な文字列の区別がつきやすい |

**推奨**：可能であれば、複数の文を含む長めの平文候補で評価すると、より信頼性の高い結果が得られます。

#### メタデータ除外のトレードオフ

メタデータ行（`shift=N` など）は評価から除外されます。これにより、メタデータ値を変更してもスコアが変動しない一貫性が保たれますが、評価対象が純粋な平文のみになるため、上記のスコア差の問題が顕在化する場合があります。
